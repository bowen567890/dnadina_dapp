...
class WithdrawStatisticsUsdt extends Card
{
    ...
    public function handle( $request)
    {
        ...
        switch ($request->get('option')) {
            default:
                ...
                $c_withdraw = $cWebsiteAnalyzeDaily->withdraw_usdt;
                
                if ($yWebsiteAnalyzeDaily) {
                    $c_withdraw_issue = $yWebsiteAnalyzeDaily->withdraw_usdt_issue;
                    $y_withdraw = $yWebsiteAnalyzeDaily->withdraw_usdt;
                    $y_withdraw_issue = $yWebsiteAnalyzeDaily->withdraw_usdt_issue;
                }
                
                $WebsiteStatistic = WebsiteStatistic::query()->where('id', 1)->first();
                $withdraw = $WebsiteStatistic->withdraw_usdt;
                $withdraw_issue = $WebsiteStatistic->withdraw_usdt_issue;
